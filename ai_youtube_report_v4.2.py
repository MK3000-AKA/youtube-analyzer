#!/usr/bin/env python3
"""
YouTube 9模块报告生成器 v4.2 - 直接调用Kimi API版
读取配置文件中的API Key，直接调用Kimi API，更稳定快速
"""
...
def load_config():
    ...
    if CONFIG_PATH.exists():
        try:
            ...
            # 如果没有找到，使用gateway_ai配置
            if not config['api_key']:
                gateway = data.get('gateway_ai', {})
                if gateway.get('provider') == 'kimi':
                    # 解析环境变量引用
                    api_key_ref = gateway.get('api_key', '')
    ...
    # 备选：从环境变量读取
    if not config['api_key']:
        config['api_key'] = os.environ.get('KIMI_API_KEY') or os.environ.get('MOONSHOT_API_KEY')
...
def call_kimi_api(prompt, timeout=60):
    ...
    try:
        ...
        data = {
            ...
            'temperature': 1.0  # k2.5 模型只支持 temperature=1.0
        }
...
请确保数据合理，只输出JSON，不要其他内容。"""
    ...
    result = call_kimi_api(prompt, timeout=60)
...
def generate_report(video_id, output_dir):
    ...
    config = load_config()
    if not config['api_key']:
        print("❌ 未找到有效的Kimi API Key，请检查 ~/.openclaw/config.yaml")
        return None, "error"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python ai_youtube_report_v4.2.py <video_id> [output_dir]")
        print("\nVersion 4.2 - 直接调用Kimi API")
        print("读取配置: ~/.openclaw/config.yaml")
        sys.exit(1)
